...
export interface GlassConfig {
  ...
  customCss: string
  /** Master switch for frosted-glass effect. */
  frosted: boolean
  /** Backdrop-filter blur radius on cards, 0..40. */
  frostBlur: number
  /** Edge refraction strength, 0..1. */
  refraction: number
  /** Glass tint color (hex). */
  tint: string
  /** Glass tint opacity, 0..1. */
  tintOpacity: number
  /** SVG feDisplacementMap scale, 0..200. */
  edgeRefractionScale: number
  /** Wallpaper brightness, 0.2..1.6. */
  brightness: number
  /** Glass material brightness, 0.2..1.6. */
  glassBrightness: number
  /** Background layer blur, 0..40. */
  bgBlur: number
}
      ...
      handler: async (req, res) => {
        ...
        if (req.method === 'PUT') {
          try {
            ...
            merged.frosted = merged.frosted === true
            merged.frostBlur = clampNumber(merged.frostBlur, 0, 40, DEFAULT_CONFIG.frostBlur)
            merged.refraction = clampNumber(merged.refraction, 0, 1, DEFAULT_CONFIG.refraction)
            merged.tint = /^#[0-9a-f]{6}$/i.test(String(merged.tint ?? '')) ? String(merged.tint).toLowerCase() : DEFAULT_CONFIG.tint
            merged.tintOpacity = clampNumber(merged.tintOpacity, 0, 1, DEFAULT_CONFIG.tintOpacity)
            merged.edgeRefractionScale = clampNumber(merged.edgeRefractionScale, 0, 200, DEFAULT_CONFIG.edgeRefractionScale)
            merged.brightness = clampNumber(merged.brightness, 0.2, 1.6, DEFAULT_CONFIG.brightness)
            merged.glassBrightness = clampNumber(merged.glassBrightness, 0.2, 1.6, DEFAULT_CONFIG.glassBrightness)
            merged.bgBlur = clampNumber(merged.bgBlur, 0, 40, DEFAULT_CONFIG.bgBlur)
...
export function bootstrapCss(config: GlassConfig): string {
  ...
  return [
    ...
    `--glass-frosted:${config.frosted ? '1' : '0'};`,
    `--glass-frost-blur:${config.frostBlur}px;`,
    `--glass-refraction:${config.refraction};`,
    `--glass-tint:${config.tint};`,
    `--glass-tint-opacity:${config.tintOpacity};`,
    `--glass-edge-refraction-scale:${config.edgeRefractionScale};`,
    `--glass-brightness:${config.brightness};`,
    `--glass-glass-brightness:${config.glassBrightness};`,
    `--glass-bg-blur:${config.bgBlur}px;`,

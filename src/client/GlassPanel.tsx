...
export function GlassPanel({ t, engine }: GlassPanelProps): JSX.Element {
  ...
  return (
    <div className={styles.panel}>
      ...
      {/* ---- liquid glass ---- */}
      <section className={styles.card}>
        <h3 className={styles.cardTitle}>{t('liquidTitle')}</h3>
        <p className={styles.hint} style={{ marginTop: 0 }}>{t('liquidSubtitle')}</p>

        <label className={styles.toggleRow}>
          <input
            type="checkbox"
            checked={config.frosted}
            onChange={(e) => update({ ...config, frosted: e.target.checked })}
          />
          <span>{t('frostedLabel')}</span>
          <span className={styles.hint}>{t('frostedHint')}</span>
        </label>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('frostBlurLabel')}</span>
            <span className={styles.value}>{config.frostBlur}px</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, frostBlur: Number(e.target.value) })}
          />
          <p className={styles.hint}>{t('frostBlurHint')}</p>
        </div>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('refractionLabel')}</span>
            <span className={styles.value}>{config.refraction.toFixed(2)}</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, refraction: Number(e.target.value) / 100 })}
          />
          <p className={styles.hint}>{t('refractionHint')}</p>
        </div>

        <div className={styles.row}>
          <span className={styles.fieldLabel}>{t('tintLabel')}</span>
          <input
            type="color"
            value={config.tint}
            onChange={(e) => update({ ...config, tint: e.target.value })}
            style={{ width: 36, height: 28, border: 'none', padding: 0, cursor: 'pointer', background: 'transparent' }}
          />
          <Input
            className={styles.grow}
            value={config.tint}
            onChange={(e) => {
              ...
              if (/^#[0-9a-f]{6}$/i.test(v) || v === '') update({ ...config, tint: v })
            }}
            placeholder="#ffffff"
          />
        </div>
        <p className={styles.hint}>{t('tintHint')}</p>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('tintOpacityLabel')}</span>
            <span className={styles.value}>{Math.round(config.tintOpacity * 100)}%</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, tintOpacity: Number(e.target.value) / 100 })}
          />
          <p className={styles.hint}>{t('tintOpacityHint')}</p>
        </div>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('edgeRefractionLabel')}</span>
            <span className={styles.value}>{config.edgeRefractionScale}</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, edgeRefractionScale: Number(e.target.value) })}
          />
          <p className={styles.hint}>{t('edgeRefractionHint')}</p>
        </div>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('brightnessLabel')}</span>
            <span className={styles.value}>{config.brightness.toFixed(2)}</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, brightness: Number(e.target.value) / 100 })}
          />
          <p className={styles.hint}>{t('brightnessHint')}</p>
        </div>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('glassBrightLabel')}</span>
            <span className={styles.value}>{config.glassBrightness.toFixed(2)}</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            onChange={(e) => update({ ...config, glassBrightness: Number(e.target.value) / 100 })}
          />
          <p className={styles.hint}>{t('glassBrightHint')}</p>
        </div>

        <div className={styles.field}>
          <div className={styles.fieldHead}>
            <span>{t('bgBlurLabel')}</span>
            <span className={styles.value}>{config.bgBlur}px</span>
          </div>
          <input
            type="range"
            className={styles.range}
            ...
            max={40}
            value={config.bgBlur}
            onChange={(e) => update({ ...config, bgBlur: Number(e.target.value) })}
          />
          <p className={styles.hint}>{t('bgBlurHint')}</p>
        </div>
      </section>
